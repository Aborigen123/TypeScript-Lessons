...
//створемо файл package.json  пишемо npm init і після цього вводимо імя package.json
//перейменуємо файл package.json на  "name": "ts-wfm-test", (це не є обовязковий но навсякий випадок щоб назви не збігалися)
//npm i --save-dev webpack typescript- встановлюємо деякі пакети як залежності для розробки
//створюємо файл webpack.config.js - стандартна назва для конфігурацій webpack
//якщо файл не глобальний то треба прописати tmp i -g webpack
//наступні 2 лінійки в webpack.config.js
// entry: './src/index.js',//створюємо папку src і переносимо наш файл туда
//в індекс html змінемо підключення на main.js в script src | файл main.js створиться сам
//webpack.config.js - закінчили конфігурувати
//після створення dist по силці буде опис як робити злиття https://github.com/s-panferov/awesome-typescript-loader
//для того щоб робити злитття треба npm install awesome-typescript-loader --save-dev і переходимо по посиланні копіюємо текст і вставляємо його в webpack.config.js
//для того щоб запустити прописуємо команду webpack
